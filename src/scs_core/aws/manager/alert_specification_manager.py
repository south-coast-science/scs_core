...
class AlertSpecificationManager(APIClient):
    ...
    def find(self, token, description_filter, topic_filter, path_filter, creator_filter):
        request = AlertSpecificationFinderRequest(description_filter, topic_filter, path_filter, creator_filter)

        response = self._http_client.get(self.__URL, headers=self._token_headers(token), params=request.params())
        self._check_response(response)

        return AlertSpecificationFinderResponse.construct_from_jdict(response.json())
    ...
    def retrieve(self, token, id):
        ...
        response = AlertSpecificationFinderResponse.construct_from_jdict(http_response.json())
...
class AlertSpecificationFinderRequest(object):
    """
    classdocs
    """
    ...
    def __str__(self, *args, **kwargs):
        return "AlertSpecificationFinderRequest:{description_filter:%s, topic_filter:%s, path_filter:%s, " \
               "creator_filter:%s}" % \
...
class AlertSpecificationFinderResponse(HTTPResponse):
    """
    classdocs
    """
    ...
    def __str__(self, *args, **kwargs):
        return "AlertSpecificationFinderResponse:{status:%s, alerts:%s, next_url:%s}" % \
