...
class CognitoManager(object):
    ...
    __URL = "https://p18hyi3w56.execute-api.us-west-2.amazonaws.com/default/CognitoManager"
    ...
    def find(self, tag_filter, exact_match, result_code, response_mode):
        request = CognitoRequest(tag_filter, exact_match, result_code, response_mode)
        ...
        return CognitoResponse.construct_from_jdict(response.json())


    # ----------------------------------------------------------------------------------------------------------------

    def __str__(self, *args, **kwargs):
        return "CognitoManager:{auth:%s}" % self.__auth


# --------------------------------------------------------------------------------------------------------------------

class CognitoRequest(object):
    """
    classdocs
    """
    ...
    def __str__(self, *args, **kwargs):
        return "CognitoRequest:{tag_filter:%s, exact_match:%s, result_code:%s, response_mode:%s}" % \
               (self.tag_filter, self.exact_match, self.result_code, self.response_mode)


# --------------------------------------------------------------------------------------------------------------------

class CognitoResponse(HTTPResponse):
    """
    classdocs
    """
    ...
    def construct_from_jdict(cls, jdict):
        ...
        mode = CognitoRequest.MODE[jdict.get('mode')]

        items = []
        if jdict.get('Items'):
            for item_jdict in jdict.get('Items'):
                item = item_jdict.get('tag') if mode == CognitoRequest.MODE.TAGS_ONLY else \
                    Cognito.construct_from_jdict(item_jdict)
                items.append(item)
    ...
    def __init__(self, status, mode, items, next_url=None):
        ...
        self.__mode = mode                              # CognitoRequest.Mode member
        self.__items = items                            # list of Cognito or string
    ...
    def __str__(self, *args, **kwargs):
        return "CognitoResponse:{status:%s, mode:%s, items:%s, next_url:%s}" % \
